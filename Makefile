...
# Зависимости пакета — с сервера роков tnt-skein, по rockspec. Рок http
# и tnt-env нужны только проверкам: живая проверка сокета поднимает
# настоящий http.server, а проверка mTLS читает каталог сертификатов
# из окружения.
.PHONY: deps
deps: ## Поставить зависимости пакета и инструменты проверок в .rocks
	...
	tt rocks install --server=https://tnt-skein.github.io/rocks --only-deps tnt-http-scm-1.rockspec
...
# Стенд живых проверок — httpbin и etcd, требующий клиентский сертификат,
# в докере. Без него три живые проверки пропускаются: гейты не должны
# зависеть от докера. Проверки сокета и враждебного сервера поднимают
# свои серверы сами и стенда не ждут.
.PHONY: stand-up
stand-up: ## Поднять httpbin и etcd с mTLS для живых проверок
	test/stand/httpbin.sh
	test/stand/etcd_mtls.sh

.PHONY: stand-down
stand-down: ## Погасить стенд живых проверок
